                    ...
                    self.render_template(
                        ...
                        {'postargs':post_args,
                         ...
                         'sig_request':duo_web.sign_request(
                             skey, ikey, self.duo_username(user)),
                         'host':host})
...
class Prompt(Handler):
    ...
    def post(self):
        ...
        duo_user = duo_web.verify_response(
            skey, self.request.get('sig_response', ''))
        if duo_user != self.duo_username(user):
...
def initialize_globals(ikey, skey, host, **kwargs):
    logging.basicConfig(
        level=logging.DEBUG,
    ...
    initialize_open_id()
    initialize_app()
    initialize_duo(ikey, skey, host)

def initialize_duo(ikey_, skey_, host_):
    global ikey, skey, host
    if ikey is None or skey is None or host is None:
        ikey = ikey_
        skey = skey_
        host = host_
